...
void light_process_omni(int idx, vec3 vertex, vec3 eye_vec, vec3 normal, float roughness, inout vec3 diffuse, inout vec3 specular) {
	...
	vec3 local_light_rel_vec = -(omni_lights[idx].light_inverse_matrix * vec4(vertex, 1.0)).xyz;
	
	float m_len = max(max(abs(local_light_rel_vec.x), abs(local_light_rel_vec.y)), abs(local_light_rel_vec.z));
	vec3 norm_l = m_len > 0.0 ? abs(local_light_rel_vec) / m_len : vec3(0.0);
	vec3 l2 = norm_l * norm_l;
	vec3 l4 = l2 * l2;
	float sq_length = m_len * sqrt(sqrt(sqrt(l4.x * l4.x + l4.y * l4.y + l4.z * l4.z)));
	
	float light_length = mix(length(local_light_rel_vec), sq_length, omni_lights[idx].shadow_extra_params.y);
...
void light_process_spot(int idx, vec3 vertex, vec3 eye_vec, vec3 normal, float roughness, inout vec3 diffuse, inout vec3 specular) {
	...
	vec3 local_light_rel_vec = -(spot_lights[idx].light_inverse_matrix * vec4(vertex, 1.0)).xyz;

	// Distance attenuation (L8 superellipsoid)
	float m_len = max(max(abs(local_light_rel_vec.x), abs(local_light_rel_vec.y)), abs(local_light_rel_vec.z));
	vec3 norm_l = m_len > 0.0 ? abs(local_light_rel_vec) / m_len : vec3(0.0);
	vec3 l2 = norm_l * norm_l;
	vec3 l4 = l2 * l2;
	...
	float light_length = mix(length(local_light_rel_vec), sq_length, spot_lights[idx].shadow_extra_params.y);
	...
	vec3 L = -local_light_rel_vec;
	
	float m_dir = max(abs(L.x), abs(L.y));
	vec2 norm_L = m_dir > 0.0 ? abs(L.xy) / m_dir : vec2(0.0);
	vec2 lxy2 = norm_L * norm_L;
	vec2 lxy4 = lxy2 * lxy2;
	float sq_dir = m_dir * sqrt(sqrt(sqrt(lxy4.x * lxy4.x + lxy4.y * lxy4.y)));
	
	vec3 L_dir = mix(L, vec3(sq_dir, 0.0, L.z), spot_lights[idx].shadow_extra_params.y);
	
	float scos = max(dot(normalize(L_dir), spot_dir), spot_cutoff);
...
#define screen_uv (gl_FragCoord.xy * screen_pixel_size)
#endif
...
float contact_shadow_compute(vec3 pos, vec3 normal, vec3 dir, float max_distance) {
    // 1. TERMINATOR CULLING:
    // If the surface faces away from the light, standard diffuse lighting handles it.
    // Tracing here only causes self-shadowing acne.
    float NdotL = dot(normal, dir);
    if (NdotL <= 0.0) return 1.0; 

    if (abs(dir.z) > 0.999) return 1.0;

    // 2. NORMAL OFFSET BIAS:
    // Lift the ray slightly off the geometry along the normal vector to prevent self-collision.
    float slope_bias = 1.0 - NdotL;
    float bias_amount = 0.005 + 0.015 * slope_bias;
    vec3 ray_start = pos + normal * bias_amount;

    vec3 ray_end = ray_start + dir * max_distance;

    // Near-Plane Clipping
    float near_plane = -0.1; 
    if (ray_end.z > near_plane) {
        float t = (near_plane - ray_start.z) / dir.z;
        ray_end = ray_start + dir * t;
    }

    vec4 clip_start = projection_matrix * vec4(ray_start, 1.0);
    vec4 clip_end = projection_matrix * vec4(ray_end, 1.0);

    vec3 uv_start = (clip_start.xyz / clip_start.w) * 0.5 + 0.5;
    vec3 uv_end = (clip_end.xyz / clip_end.w) * 0.5 + 0.5;

    // Dynamic Thickness Calculation
    vec4 bias_clip = projection_matrix * vec4(ray_start + vec3(0.0, 0.0, max_distance * 0.5), 1.0);
    float bias_depth = (bias_clip.z / bias_clip.w) * 0.5 + 0.5;
    float thickness_tolerance = abs(bias_depth - uv_start.z);
    thickness_tolerance = clamp(thickness_tolerance, 0.0001, 0.05);

    vec3 delta_uv = uv_end - uv_start;
    vec2 delta_pixels = abs(delta_uv.xy / screen_pixel_size);
    
    float steps = max(delta_pixels.x, delta_pixels.y);
    if (steps < 1.0) return 1.0;
    
    float max_steps = 64.0;
    steps = min(steps, max_steps); 
    vec3 step_uv = delta_uv / steps;

    // Dither jitter
    vec2 frag_coord = uv_start.xy / screen_pixel_size;
    float jitter = interleaved_gradient_noise(frag_coord);

    // Step forward 1.5 pixels + jitter to completely exit the originating texel
    vec3 current_uv = uv_start + step_uv * (1.5 + jitter);
    ...
    for (float i = 0.0; i < 64.0; i += 1.0) {
        ...
        if (any(lessThan(current_uv.xy, vec2(0.0))) || any(greaterThan(current_uv.xy, vec2(1.0)))) {
            break;
        }

        float depth_sampled = texture(depth_buffer, current_uv.xy).r;
        float depth_diff = current_uv.z - depth_sampled;

        // Depth epsilon prevents surface grazing float precision errors
        if (depth_diff > 0.00005 && depth_diff < thickness_tolerance) {
            float ratio = (i + jitter) / steps;
            shadow = min(pow(ratio, 4.0), 1.0);
            break;
        }
    ...
    // Smoothly fade out the shadow at grazing angles to prevent a hard, pixelated edge
    return mix(1.0, shadow, smoothstep(0.0, 0.15, NdotL));
}
...
float dither_r2(vec2 pos) {
	...
	cell_hash = (cell_hash ^ (cell_hash >> 16u)) * 0x27d4eb2du;
	float jitter = float(cell_hash & 0xFFu) / 256.0;
...
float get_shadow_dither(vec2 pos, float mode_f, float temporal_offset) {
	int mode = int(mode_f + 0.5);
	float d;
	if (mode == 1) {
		d = dither_bayer16(pos);
	} else if (mode == 2) {
		d = dither_r2(pos);
	} else if (mode == 3) {
	...
	return fract(d + temporal_offset);
}

float quick_hash(vec2 pos) {
	return dither_ign(pos);
}




float sample_shadow(highp sampler2DShadow shadow, vec2 shadow_pixel_size, vec2 pos, float depth, vec4 clamp_rect, float shadow_blur, float shadow_dither_mode, float shadow_temporal_offset) {
    shadow_pixel_size *= shadow_blur;
	...
	vec2 dither = (-vec2(0.5) + dither_val) * shadow_pixel_size;
	pos += dither;
#endif
	...
	// PCF25 (highest quality).
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x * 3.0, 0.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x * 3.0, 0.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(0.0, shadow_pixel_size.y * 3.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(0.0, -shadow_pixel_size.y * 3.0), depth, 1.0));
	...
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x * 2.0, -shadow_pixel_size.y), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x * 2.0, -shadow_pixel_size.y), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x * 2.0, shadow_pixel_size.y), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x * 2.0, shadow_pixel_size.y), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x, -shadow_pixel_size.y * 2.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x, -shadow_pixel_size.y * 2.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x, shadow_pixel_size.y * 2.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x, shadow_pixel_size.y * 2.0), depth, 1.0));
	return avg * (1.0 / 25.0);
...
#ifndef SHADOW_MODE_PCF_LOW //ubershader-runtime
#ifdef SHADOW_MODE_PCF_HIGH //ubershader-runtime
	// PCF13 (high quality) - Optimized with textureGather
	avg += textureProj(shadow, vec4(pos + vec2(shadow_pixel_size.x * 2.0, 0.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(-shadow_pixel_size.x * 2.0, 0.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(0.0, shadow_pixel_size.y * 2.0), depth, 1.0));
	avg += textureProj(shadow, vec4(pos + vec2(0.0, -shadow_pixel_size.y * 2.0), depth, 1.0));
	...
	// Gather the 3x3 inner neighborhood using four 2x2 textureGather calls.
	vec4 g0 = textureGather(shadow, pos + vec2(-0.5, -0.5) * shadow_pixel_size, depth);
	vec4 g1 = textureGather(shadow, pos + vec2( 0.5, -0.5) * shadow_pixel_size, depth);
	vec4 g2 = textureGather(shadow, pos + vec2(-0.5,  0.5) * shadow_pixel_size, depth);
	vec4 g3 = textureGather(shadow, pos + vec2( 0.5,  0.5) * shadow_pixel_size, depth);

	avg += dot(g0, vec4(1.0)) + dot(g1, vec4(1.0)) + dot(g2, vec4(1.0)) + dot(g3, vec4(1.0));
	
	// Total accumulated: 1 (initial center) + 4 (outer cross) + 16 (inner 3x3 gathers) = 21 samples.
	return avg * (1.0 / 21.0);
#endif //ubershader-runtime
#endif //ubershader-runtime
...
#ifdef SHADOW_MODE_PCF_LOW //ubershader-runtime
#ifndef SHADOW_MODE_PCF_HIGH //ubershader-runtime
	// 12-Point Golden Ratio Vogel Disk
	float noise_angle = get_shadow_dither(gl_FragCoord.xy, shadow_dither_mode, shadow_temporal_offset) * 6.28318530718;

	float s_noise = sin(noise_angle);
	float c_noise = cos(noise_angle);
	mat2 rot = mat2(c_noise, -s_noise, s_noise, c_noise);

	const vec2 vogel_samples[12] = vec2[](
		vec2(0.2041, 0.0000),
		vec2(-0.2605, 0.2389),
		vec2(0.0401, -0.4547),
		vec2(0.3340, 0.4239),
	...
	// Center sample
	float total_shadow = textureProj(shadow, vec4(pos, depth, 1.0));

	// 12 distributed taps
	for (int i = 0; i < 12; i++) {
		vec2 sample_offset = (rot * vogel_samples[i]) * shadow_pixel_size;
		total_shadow += textureProj(shadow, vec4(pos + sample_offset, depth, 1.0));
	}

	// Average 13 total samples (1 center + 12 taps)
	return total_shadow * (1.0 / 13.0);
#endif //ubershader-runtime
#endif //ubershader-runtime
    ...
    vec2 tex_size = vec2(textureSize(shadow, 0));
    vec2 inv_tex_size = 1.0 / tex_size;

    float blur_radius = 2.0;
    vec2 radius = inv_tex_size * blur_radius;

    const int SAMPLES = 6;
    const float GOLDEN_ANGLE = 2.3999632; // ~137.5 degrees

    float shadow_val = 0.0;
    for (int i = 0; i < SAMPLES; i++) {
        // Distribute taps evenly by area across the disk
        float r = sqrt((float(i) + 0.5) / float(SAMPLES));
        float theta = float(i) * GOLDEN_ANGLE;
        vec2 tap = vec2(cos(theta), sin(theta)) * r;

        shadow_val += textureProj(shadow, vec4(pos + tap * radius, depth, 1.0));
    }
    shadow_val /= float(SAMPLES);

    // Threshold using screen-space width for that crisp Illustrator/vector line
    float dist = shadow_val - 0.5;
    float fw = max(fwidth(shadow_val), 0.001);
...
void light_process_omni(int idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 binormal, vec3 tangent, vec3 albedo, vec3 transmission, float roughness, float metallic, float specular, float rim, float rim_tint, float clearcoat, float clearcoat_gloss, float anisotropy, float p_blob_intensity, inout vec3 diffuse_light, inout vec3 specular_light, inout float alpha) {
	...
	if (omni_lights[idx].light_params.w > 0.5) {
		...
		float NdotL_omni = max(dot(normal, normalize(light_rel_vec)), 0.0);
		float slope_factor_omni = clamp(1.0 - NdotL_omni, 0.0, 1.0);
		vec3 omni_shadow_vertex = vertex + normal * (0.02 * slope_factor_omni);

		highp vec3 splane = (omni_lights[idx].shadow_matrix * vec4(omni_shadow_vertex, 1.0)).xyz;
		float shadow_len = length(splane); // MUST strictly be Euclidean distance!
		...
		if (shadow > 0.01 && omni_lights[idx].shadow_color_contact.a > 0.0) {
			float contact_shadow = contact_shadow_compute(vertex, normal, normalize(light_rel_vec), min(light_length, omni_lights[idx].shadow_color_contact.a));
...
void light_process_spot(int idx, vec3 vertex, vec3 eye_vec, vec3 normal, vec3 binormal, vec3 tangent, vec3 albedo, vec3 transmission, float roughness, float metallic, float specular, float rim, float rim_tint, float clearcoat, float clearcoat_gloss, float anisotropy, float p_blob_intensity, inout vec3 diffuse_light, inout vec3 specular_light, inout float alpha) {
	...
	if (spot_lights[idx].light_params.w > 0.5) {
		// there is a shadowmap
		float NdotL_spot = max(dot(normal, normalize(light_rel_vec)), 0.0);
		float slope_factor_spot = clamp(1.0 - NdotL_spot, 0.0, 1.0);
		vec3 spot_shadow_vertex = vertex + normal * (0.02 * slope_factor_spot);
		...
		if (shadow > 0.01 && spot_lights[idx].shadow_color_contact.a > 0.0) {
			float contact_shadow = contact_shadow_compute(vertex, normal, normalize(light_rel_vec), min(light_length, spot_lights[idx].shadow_color_contact.a));
			shadow = min(shadow, contact_shadow);
		}
#endif //ubershader-runtime
...
FRAGMENT_SHADER_CODE
	...
	if (depth_z < value) {
		...
		if (use_blend) {
			shadow = mix(shadow, sample_shadow(directional_shadow, directional_shadow_pixel_size, pssm_coord2.xy, pssm_coord2.z, light_clamp, shadow_blur, shadow_dither_mode, shadow_temporal_offset), pssm_blend);
		}
#endif //ubershader-runtime

#ifdef USE_CONTACT_SHADOWS //ubershader-runtime
		if (shadow > 0.01 && shadow_color_contact.a > 0.0) {
			float contact_shadow = contact_shadow_compute(vertex, normal, -light_direction_attenuation.xyz, shadow_color_contact.a);
			shadow = min(shadow, contact_shadow);
		}
#endif //ubershader-runtime
